 ...
 * @author DELL
 */
public class Provider_Delete_StaffController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean check = false ; 
            String staffID = request.getParameter("Provider_StaffID") ; 
            HttpSession session = request.getSession() ; 
            ProviderDTO provider = (ProviderDTO) session.getAttribute("LOGIN_PROVIDER") ; 
            ProviderDAO providerDAO = new ProviderDAO() ; 
            check = providerDAO.deleteStaff(provider, staffID) ; 
            if (check == true) {
                url = SUCCESS ; 
                listService = providerDAO.loadListStaff(provider);  
                session.setAttribute("Provider_ListStaff", listService); 
            }
        } catch (Exception e) {
        }finally {
